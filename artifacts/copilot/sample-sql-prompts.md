**Prompt:** 

"Given the restaurant_events table with columns event_time, location_id, event_type, and time_to_activation_hours, generate a SQL query that returns median and p90 time_to_activation_hours by week for the last 90 days."

**Prompt:** 

"Create a query that estimates ops hours saved by automation. Assume manual_message_avg_minutes = 10 and automated_message_avg_minutes = 1.5. Use order_messages table with automated boolean and created_at timestamp."
